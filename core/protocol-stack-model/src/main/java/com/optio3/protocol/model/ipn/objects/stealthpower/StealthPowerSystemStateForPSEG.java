...
public enum StealthPowerSystemStateForPSEG implements TypedBitSet.ValueGetter
{
    // @formatter:off
    ...
    SP_OFF_NOCHG_INV(13); // Engine running, not charging, inverter on
    // @formatter:on

    private final byte m_encoding;

    StealthPowerSystemStateForPSEG(int encoding)
    {
        m_encoding = (byte) encoding;
    }

    @HandlerForDecoding
    public static StealthPowerSystemStateForPSEG parse(byte value)
    {
        for (StealthPowerSystemStateForPSEG t : values())
        {
