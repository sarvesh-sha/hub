...
public class ReadInputRegisters extends ApplicationPDU
{
    public static class Response extends ApplicationPDU.Response
    {
        ...
        protected void validateAfterDecoding()
        {
            if (results == null)
            {
                throw ModbusDecodingException.newException("Invalid ReadInputRegisters.Response");
            }

            if (count != 2 * results.length)
            {
                throw ModbusDecodingException.newException("Mot enough values for ReadInputRegisters.Response: %d != %d", count, results.length);
            }
        }
    }
    ...
    protected void validateAfterDecoding()
    {
        ...
        {
            throw ModbusDecodingException.newException("Invalid ReadInputRegisters");
        }
    }
}
