/*
 * Copyright (C) 2017-, Optio3, Inc. All Rights Reserved.
 *
 * Proprietary & Confidential Information.
 *
 * Stock Order
 * Order stock and view your company's order history.<p>You can use these endpoints to place orders and retrieve&#58;<ul> <li>Your company's order history</li> <li>The details of a specific order</li> <li>A list of operators your company can order stock from</li> <li>A list of the type of stock your company can order from each operator</li> <li>A spreadsheet listing every subscriber included in a specific order</li> </ul>
 *
 * OpenAPI spec version: 1.0.0
 *
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

package com.optio3.infra.pelion.api;

import java.io.InputStream;
import java.util.List;

import javax.ws.rs.GET;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;

import com.optio3.infra.pelion.model.ApiResults;
import com.optio3.infra.pelion.model.StockOrders;

@Path("/")
public interface StockOrdersApi
{
    @GET
    @Path("/stock-orders")
    @Produces({ "application/json" })
    public ApiResults<List<StockOrders>> stockOrdersGet();

    @GET
    @Path("/stock-orders/{orderId}")
    @Produces({ "application/json" })
    public ApiResults<List<StockOrders>> stockOrdersOrderIdGet(@PathParam("orderId") String orderId);

    @GET
    @Path("/stock-orders/{orderId}/subscribers/download")
    @Produces({ "application/octet-stream" })
    public InputStream stockOrdersOrderIdSubscribersDownloadGet(@PathParam("orderId") String orderId);
}
