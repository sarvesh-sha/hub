...
import {ITreeNode, ITreeNodeFilterSubmit} from "framework/ui/dropdowns/filterable-tree.component";
import {smartTreeWidth} from "framework/ui/forms/select.component";
    ...
    readonly onNewTextFn: ITreeNodeFilterSubmit<string> = (value: string) =>
    {
        ...
        if (this.editValues.indexOf(value) < 0)
        {
            ...
            this.editValues.sort((a,
                                  b) => UtilsService.compareStrings(a, b));
    ...
    toggleDropdown(overlayOrigin?: CdkOverlayOrigin)
    {
        if (!this.disabled && !this.readonly)
        {
            ...
            if (this.autoSizeDropdown)
            {
                ...
                this.overlayConfig.width    = Math.max(minWidth, smartTreeWidth(this.nodes, this.autoSizeThreshold));
