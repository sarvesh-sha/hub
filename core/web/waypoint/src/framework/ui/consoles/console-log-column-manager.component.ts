...
{
    public dialogConfig: OverlayConfig;
    ...
    {
        super(inj);
        this.dialogConfig = new OverlayConfig();
        this.dialogConfig.onTopDraggable();
    ...
    public updatePristineState(): void
    {
        if (this.columns)
        {
            this.pristine = this.m_activeConfig.length === this.columns.length &&
                            this.m_activeConfig.every((config,
                                                       idx) => config.equals(this.columns[idx]));

            this.isDefault = this.m_defaultConfig.length === this.columns.length &&
                             this.m_defaultConfig.every((config,
                                                         idx) => config.equals(this.columns[idx]));
        }
        else
        {
