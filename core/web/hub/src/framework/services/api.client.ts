...
export class ApiClient
{
                             ...
                             requestOptions: ApiRequest): Promise<T>
    {
        ...
        {
            let context = `${requestOptions.method} ${path}`;
            ...
            switch (error.status)
            {
                ...
                case 401:
                    console.error(`HTTP_ERROR: Status code ${error.status} on ${context}`);
            ...
            if (error.status)
            {
                ...
                throw {
                    code   : "HTTP_ERROR",
                    message: `Status code ${error.status} on ${context}`,
                    context: context
                };
            }
            ...
            throw {
                ...
                message: `An unexpected system error occurred for ${context}`,
                context: context
            };
        }
...
export class ApiRequest
{
    ...
    private static setParam(params: Lookup<string | string[]>,
                            name: string,
                            value: string): void
    {
