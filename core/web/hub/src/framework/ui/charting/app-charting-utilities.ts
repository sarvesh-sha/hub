...
export class ChartPanel
{
    ...
    chips: ChartChip[]                 = [];
    topPadding: number                 = 0;
    gridColor: string                  = "rgb(102, 102, 102)";
    zone: string;
    hideSideAxes: boolean;
    hideBottomAxis: boolean;
    hideHighlights: boolean;
    showAlerts: boolean;
    targetSource: ChartPointSource<any>;

    establishGroupForGrid()
    {
        let gridGroup: ChartGroup;
        for (let group of this.groups)
        {
            if (group.useAsLeftValueAxis)
            {
                gridGroup = group;
                break;
            }
            else if (group.useAsRightValueAxis)
            {
                gridGroup = group;
            }
        }

        gridGroup ||= this.groups[0];
        if (gridGroup) gridGroup.useForGrid = true;
    }
}
...
export class CanvasRenderer
{
    ...
    public static renderGrid(canvas: CanvasRenderingContext2D,
                             ...
                             xTickValues: number[],
                             yTickValues: number[],
                             color: string       = "rgb(102, 102, 102)",
                             globalAlpha: number = 0.333,
