...
const colorOptionPalettes: Lookup<Lookup<ColorOption>> = {
    ...
    "Progress Colors": {
        progressred   : new ColorOption("progressred", "#FF5F5B", "Progress Red"),
        progressorange: new ColorOption("progressorange", "#ee9f0d", "Progress Orange"),
        progressgreen : new ColorOption("progressgreen", "#3bb528", "Progress Green")
    }
};
...
const colorPalettes = {
    ...
    "Progress Colors": new ColorPalette("Progress Colors", colorOptionPalettes["Progress Colors"])
};
...
export class ChartColorUtilities
{
    ...
    /**
     * For the given color (input), pick one if the two given colors (colorA, colorB) that has more contrast.
     * @param {number | string} input - The color to compute contrast against.
     * @param {number | string} [colorA="white"] - The first color to compare against the input color.
     * @param {number | string} [colorB="black"] - The second color to compare against the input color.
     * @param {number} [skew=0.5] - Ranges from 0-1, used to skew in preference of colorA vs colorB, even if one has more contrast.
     */
    public static bestContrastBetween(input: string | number,
                                      colorA: string | number = "white",
                                      colorB: string | number = "black",
                                      skew: number            = 0.5): chroma.Color
    {
        let color = this.safeChroma(input);
        let a     = this.safeChroma(colorA);
        let b     = this.safeChroma(colorB);

        skew *= 2;
        let skewA = Math.min(1, skew);
        let skewB = Math.min(1, 2 - skew);

        if (skewA <= 0) return a;
        if (skewB <= 0) return b;

        let aContrast = chroma.contrast(a, color) * skewB;
        let bContrast = chroma.contrast(b, color) * skewA;
        return aContrast >= bContrast ? a : b;
    }

    public static progressColorFn      = (value: number) => this.progressColorMapper.getColor(value);
    private static progressColorMapper = new StepwiseColorMapper([
                                                                     new ColorGradientStop(0, this.getColorById("Progress Colors", "progressred").hex),
                                                                     new ColorGradientStop(1, this.getColorById("Progress Colors", "progressorange").hex),
                                                                     new ColorGradientStop(100, this.getColorById("Progress Colors", "progressgreen").hex),
                                                                     new ColorGradientStop(100, this.getColorById("Progress Colors", "progressgreen").hex)
                                                                 ]);
}
