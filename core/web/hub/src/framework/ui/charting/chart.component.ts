    ...
    private redrawCanvas()
    {
        if (this.panelsReady && this.stateIsReadyForCanvas())
        {
            ...
            if (this.stateChanged)
            {
                ...
                for (let panel of this.computedPanels)
                {
                    hasLeftAxis ||= !!panel.rawPanel.leftAxis.label;
                    hasRightAxis ||= !!panel.rawPanel.rightAxis.label;
...
class ProcessedPanel
{
    ...
    private groupForGrid: ProcessedGroup;
    private groupForLeftAxis: ProcessedGroup;
    private groupForRightAxis: ProcessedGroup;
    private clipForBottomAxis: ChartClipArea;

    private sourceForZoom: ProcessedDataSource;

    private sourceForLeftHighlight: ProcessedDataSource;
    private sourceForRightHighlight: ProcessedDataSource;
    private sourceForExpandedPoints: ProcessedDataSource;

    private highlightPointLeft: ChartPointWithTransform<any>;
    private highlightPointRight: ChartPointWithTransform<any>;
                ...
                public readonly rawPanel: ChartPanel)
    {
        ...
        {
            if (!this.groupForGrid && processedGroup.group.useForGrid) this.groupForGrid = processedGroup;
            ...
            {
                if (!this.sourceForLeftHighlight && processedGroup.group.useAsLeftValueAxis) this.sourceForLeftHighlight = source;
                if (!this.sourceForRightHighlight && processedGroup.group.useAsRightValueAxis) this.sourceForRightHighlight = source;
                if (!this.sourceForExpandedPoints && source.dataSource.showExpandedPoints) this.sourceForExpandedPoints = source;
            }
        }
    ...
    {
        this.clearTooltip();
        this.highlightPointLeft  = null;
        this.highlightPointRight = null;
    ...
    computeReservedHeight(): number
    {
        ...
        if (this.groupForGrid && !this.rawPanel.hideBottomAxis) res += categoryAxisSize;
                  ...
                  topPadding: number): number
    {
        ...
        if (this.groupForGrid && !this.rawPanel.hideBottomAxis)
        {
            this.clipForBottomAxis = new ChartClipArea(x, y, width, categoryAxisSize);
            y += categoryAxisSize;
        }
                              ...
                              sources: ProcessedDataSource[])
    {
        for (let processedDataSource of sources)
        {
            if (processedDataSource.dataSource.fillArea)
            {
                ...
                processedDataSource.renderArea(canvas, processedDataSource.group.transform, color, 0.5);
            }
        }
    }
        ...
        for (let processedDataSource of sources)
        {
            if (processedDataSource.dataSource.showPoints)
            {
                for (let range of processedDataSource.dataSource.ranges)
                {
                    ...
                    CanvasRenderer.renderPoints(helper, view, processedDataSource.dataSource.baseRadius, overrideColor || processedDataSource.dataSource.color, overrideColor, overrideColor, true);
                }
            }
        }
    }
    ...
    private renderGrid(helper: ChartHelpers)
    {
        this.clipForDataArea.applyClipping(helper.canvas, () =>
        {
            if (!this.rawPanel.hideSideAxes || !this.rawPanel.hideBottomAxis)
            {
                CanvasRenderer.renderGridOutline(helper.canvas, this.clipForDataArea, this.rawPanel.gridColor, true, !this.rawPanel.hideBottomAxis);
            }

            if (this.groupForGrid)
            {
                this.groupForGrid.renderGrid(helper, this.clipForDataArea, this.majorTicksX);
            }
        });
    }
                       ...
                       font: ChartFont)
    {
        ...
        if (this.groupForGrid && !this.rawPanel.hideBottomAxis)
        {
            ...
            this.clipForBottomAxis?.applyClipping(canvas, () =>
            {
                ...
                for (let tick of this.majorTicksX)
                {
                    ...
                    let x         = this.groupForGrid.fromTimestampToXCoordinate(tick.moment);
                    let prevStyle = font.style;
                    if (tick.bold)
                    {
                        font.style = "bold";
                    }
                    helper.drawTextInBox(font, TextPlacement.Center, TextOrientation.Horizontal, text, "black", x + 2, this.clipForBottomAxis.y, BoxAnchor.Top, 5);
    ...
    private renderHighlights(helper: ChartHelpers,
                             font: ChartFont)
    {
        if (this.rawPanel.hideHighlights) return;

        const hasLeft  = this.highlightPointLeft && this.sourceForLeftHighlight;
        const hasRight = this.highlightPointRight && this.sourceForRightHighlight;
        if (hasLeft || hasRight)
        {
            const canvas = helper.canvas;

            const highlightPoint = hasLeft ? this.highlightPointLeft : this.highlightPointRight;
            const pixel          = highlightPoint.toPixel();
            this.clipForPanel.applyClipping(canvas, () =>
            {
                ...
                canvas.lineTo(this.clipForDataArea.right, pixel.y);
                canvas.stroke();

                canvas.lineWidth = 1;
                canvas.moveTo(pixel.x, this.clipForDataArea.y);
                canvas.lineTo(pixel.x, this.clipForDataArea.bottom);
                canvas.stroke();
            });

            if (hasRight) this.renderHighlightY(helper, font, false, this.highlightPointRight);
            if (hasLeft) this.renderHighlightY(helper, font, true, this.highlightPointLeft);

            if (this.clipForBottomAxis)
            {
                this.clipForBottomAxis.applyClipping(canvas, () =>
                {
                    const xLabel             = MomentHelper.friendlyFormatVerboseUS(highlightPoint.point.xAsMoment, true, this.withSubSecondX);
                    const halfHighlightWidth = helper.placeTextInBox(font, TextPlacement.Center, TextOrientation.Horizontal, xLabel, pixel.x, this.clipForBottomAxis.y, BoxAnchor.Top, 5).width / 2;
                    const x                  = UtilsService.clamp(this.clipForPanel.left + halfHighlightWidth, this.clipForPanel.right - halfHighlightWidth, pixel.x);
                    helper.drawTextInBox(font, TextPlacement.Center, TextOrientation.Horizontal, xLabel, "white", x, this.clipForBottomAxis.y, BoxAnchor.Top, 5, highlightColor);
                });
            }
        }
    }

    private renderHighlightY(helpers: ChartHelpers,
                             font: ChartFont,
                             left: boolean,
                             source: ChartPointWithTransform<any>)
    {
        const canvas = helpers.canvas;
        const pixel  = source.toPixel();

        this.clipForPanel.applyClipping(canvas, () =>
        {
            const yLabel = source.point.numberValue.toFixed(2);
            const x      = left ? this.clipForDataArea.left : this.clipForDataArea.right;
            const anchor = left ? BoxAnchor.Left : BoxAnchor.Right;

            const halfHighlightHeight = helpers.placeTextInBox(font, TextPlacement.Center, TextOrientation.Horizontal, yLabel, x, pixel.y, anchor, 5).height / 2;
            ...
            helpers.drawTextInBox(font, TextPlacement.Center, TextOrientation.Horizontal, yLabel, "white", x, y, anchor, 5, highlightColor);
        });
    }
                                                ...
                                                e: WindowEventMap[K]): boolean
    {
        ...
        switch (type)
        {
            case "click":
            {
                ...
                const highlightSource = this.sourceForLeftHighlight || this.sourceForRightHighlight;
                if (highlightSource && this.clipForDataArea.hitCheck(x, y))
                {
                    ...
                    {
                        let point        = highlightSource.fromPixelToSample(x, y);
    ...
    {
        let redraw = false;
        if (this.highlightPointLeft != null)
        {
            this.highlightPointLeft = null;
            redraw                  = true;
        }

        if (this.highlightPointRight != null)
        {
            this.highlightPointRight = null;
            redraw                   = true;
        }
                    ...
                    y: number): boolean
    {
        ...
        if (this.clipForDataArea.hitCheck(x, y))
        {
            ...
            if (this.sourceForLeftHighlight || this.sourceForRightHighlight)
            {
                let maxSampleDistance = 8;
                let targetChartSource = this.targetSource?.dataSource;

                let point = this.sourceForRightHighlight?.fromPixelToSample(x, y);
                if (point && (!this.highlightPointRight || this.highlightPointRight.point != point.point))
                {
                    this.highlightPointRight = point;
                    redraw                   = true;
                }

                point = this.sourceForLeftHighlight?.fromPixelToSample(x, y);
                if (point && (!this.highlightPointLeft || this.highlightPointLeft.point != point.point))
                {
                    this.highlightPointLeft = point;
                    redraw                  = true;
                }
                ...
                const tooltipPoint = this.getClosestTooltipPoint(this.highlightPointLeft || this.highlightPointRight, x, y, targetChartSource, true);
                if (this.setTooltipPoint(tooltipPoint)) redraw = true;
            }
        }
        else
        {
            if (this.highlightPointLeft || this.highlightPointRight || this.tooltipPoint)
            {
                this.reset();
                redraw = true;
            }
...
export class ProcessedGroup
{
               ...
               ticksX: ChartTimelineTick[])
    {
        ...
        CanvasRenderer.renderGrid(helper.canvas, this.transform, inner.x, inner.right, inner.y, inner.bottom, timestamps, this.m_majorTicksY, this.panel.rawPanel.gridColor);
    }
...
export class ProcessedDataSource
{
    ...
    {
        canvas.save();
        canvas.lineWidth   = this.dataSource.lineWidth;
