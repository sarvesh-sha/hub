...
import {AbstractType, AfterViewInit, ChangeDetectorRef, Directive, EventEmitter, Injectable, InjectionToken, Injector, Input, OnDestroy, OnInit, SecurityContext, SimpleChanges, Type} from "@angular/core";
...

{
    ...
    private viewStateDefaultCache: Lookup<any> = {};
    ...
    protected inject<T>(token: Type<T> | InjectionToken<T> | AbstractType<T>): T
    {
    ...
    protected async waitUntilTrue(retries: number,
                                  callback: () => boolean): Promise<boolean>
    {
        while (retries-- > 0)
        {
            if (this.wasDestroyed()) return false;

            if (callback()) return true;

            await Future.delayed(20);
        }
    ...
    protected subscribeToObservableUntil<T>(emitter: rx.Observable<T>,
                                            generatorOrNext: (val: T) => void,
                                            until: () => boolean,
                                            error?: (err: any) => void,
                                            complete?: () => void): rx.Subscription
    {
        ...
        return this.subscribeToObservable(emitter, compositeFn, error, complete);
    }
                                              ...
                                              mouseUp: boolean) => void)
    {
        ...
        this.subscribeToObservable(mousedrag, (e: MouseEvent) => callback(e, false, false));

        this.subscribeToObservable(mouseup, (e: MouseEvent) => callback(e, false, true));
    }
                                              ...
                                              touchEnd: boolean) => void)
    {
        ...
        const touchcancel = fromEvent(document.documentElement, "touchcancel");
        const touchFinish = merge(touchend, touchcancel);

        const touchdrag = touchstart.pipe(mergeMap((e: TouchEvent) =>
                                                   {
                                                       callback(e, true, false);
                                                       return touchmove.pipe(takeUntil(touchFinish));
                                                   }));
        this.subscribeToObservable(touchdrag, (e: TouchEvent) => callback(e, false, false));

        this.subscribeToObservable(touchFinish, (e: TouchEvent) => callback(e, false, true));
    }
    ...
    public async confirmOperation(text: string | SafeHtml): Promise<boolean>
    {
        // Don't remove the lazy import, there's a cycle between base component and overlay component.
        let DialogConfirmComponent = await import("framework/ui/dialogs/dialog-confirm.component").then(m => m.DialogConfirmComponent);
...
export function fromEvent<K extends keyof WindowEventMap>(target: any,
