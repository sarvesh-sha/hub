...
import {CdkOverlayOrigin, ComponentType, ConnectedPosition, Overlay, OverlayConfig as CdkOverlayConfig, OverlayRef, OverlaySizeConfig, PositionStrategy} from "@angular/cdk/overlay";
import {ScrollStrategy} from "@angular/cdk/overlay/scroll/scroll-strategy";
import {CdkPortal, CdkPortalOutlet, ComponentPortal, Portal, TemplatePortal} from "@angular/cdk/portal";
import {Component, ComponentRef, ContentChild, ElementRef, EventEmitter, Injector, Input, OnInit, Output, TemplateRef, Type, ViewChild, ViewContainerRef} from "@angular/core";
...
@Component({
               ...
               animations : [
                   trigger("transformDropdown", [
                       ...
                       transition("* => enter", animate(`${OverlayComponent.animationDuration}ms ease-out`)),
                       transition("enter => *", animate(`${OverlayComponent.animationDuration}ms ease-in`))
                   ]),
...
export class OverlayComponent extends BaseComponent implements OnInit
{
    ...
    @ViewChild("test_close", {read: ElementRef}) test_close: ElementRef;
    ...
    public static openRef<D extends WithTestId, R>(base: BaseComponent,
                                                   componentRef: ComponentType<any>,
                                                   config: OverlayDialogConfig<D>): OverlayDialogRef<R>
    {
        if (!config.config) config.config = new OverlayConfig();
        if (config.data) config.data.optio3TestId = config.config.optio3TestId;
                             ...
                             config: OverlayDialogConfig<D>): Promise<R>
    {
        ...
        let closed = this.waitForClose(ref);

        let router           = base.injector.get(Router);
        let navigatedAwaySub = router.events.subscribe((val) =>
                                                       {
        ...
        closed.then(() => navigatedAwaySub.unsubscribe());
    ...
    public static waitForClose<R>(ref: OverlayDialogRef<R>): Promise<R>
    {
        ...
        ref.afterClose()
           .subscribe((value) => result.resolve(value));

        return result;
    }

    private static s_dialogs = new Set<OverlayComponent>();

    public static findDialogComponent<T>(type: ComponentType<T>): T
    {
        return this.findDialogComponentsHelper(type, false)[0];
    }

    public static findDialogComponents<T>(type: ComponentType<T>): T[]
    {
        return this.findDialogComponentsHelper(type, true);
    }

    private static findDialogComponentsHelper<T>(type: ComponentType<T>,
                                                 onlyFirst: boolean): T[]
    {
        let components: T[] = [];
        for (let comp of OverlayComponent.s_dialogs)
        {
            if (comp?.portalOutlet?.attachedRef instanceof ComponentRef)
            {
                const instance = comp.portalOutlet.attachedRef.instance;
                if (instance instanceof type)
                {
                    components.push(instance);
                    if (onlyFirst) return components;
                }
            }
        }
                                          ...
                                          config: OverlayDialogConfig<any>): OverlayComponent
    {
        ...
        OverlayComponent.s_dialogs.add(overlayComponent);

        overlayComponent.config     = config.config;
        overlayComponent.m_isDialog = true;
        overlayComponent.closed.subscribe(() =>
                                          {
                                              OverlayComponent.s_dialogs.delete(overlayComponent);
                                              overlayRef.detach();
                                          });
    ...
    resize(sizeConfig: OverlaySizeConfig): void
    {
        if (this.overlay)
        {
            this.overlay.updateSize(sizeConfig);
        }
    }
    ...
    pullIntoView(timeoutDelay?: number)
    {
        if (this.isOpen && this.handle)
        {
            ...
            {
                setTimeout(() => this.pullIntoViewHelper(), timeoutDelay);
            }
        }
    }
    ...
    private buildConfig()
    {
        if (!this.m_config)
        {
            ...
            if (this.overlayOrigin) this.m_config.dropdownDefaults();
        }
        ...
        let cdkConfig = new CdkOverlayConfig({
                                                 ...
                                                 scrollStrategy  : this.m_config.scrollStrategy
                                             });
        ...
        else if (!cdkConfig.scrollStrategy)
        {
...
export class OverlayConfig
{
    ...
    public static newInstance(config?: Partial<OverlayConfig>): OverlayConfig
    {
        return new OverlayConfig().adoptConfig(config);
    }

    public static onTopDraggable(config?: Partial<OverlayConfig>): OverlayConfig
    {
        return new OverlayConfig().onTopDraggable()
                                  .adoptConfig(config);
    }

    public static dialog(config?: Partial<OverlayConfig>): OverlayConfig
    {
        return new OverlayConfig().dialogDefaults()
                                  .adoptConfig(config);
    }

    public static dropdown(config?: Partial<OverlayConfig>): OverlayConfig
    {
        return new OverlayConfig().dropdownDefaults()
                                  .adoptConfig(config);
    }

    public static wizard(config?: Partial<OverlayConfig>): OverlayConfig
    {
        return new OverlayConfig().wizardDefaults()
                                  .adoptConfig(config);
    }

    public static importExport(config?: Partial<OverlayConfig>): OverlayConfig
    {
        return new OverlayConfig().importExportDefaults()
                                  .adoptConfig(config);
    }

    private readonly id = OverlayConfig.id++;
    ...
    {
        if (classes?.length) this.m_containerClasses.push(...classes);
    }
    ...
    private adoptConfig(config: Partial<OverlayConfig>): OverlayConfig
    {
        Object.assign(this, config || {});
        return this;
    }

    onTopDraggable(): OverlayConfig
    {
    ...
    dropdownDefaults(): OverlayConfig
    {
        this.showBackdrop             = true;
    ...
    wizardDefaults(): OverlayConfig
    {
        ...
        this.isDraggable              = false;
        this.coverAnchorWhenDisplayed = false;
        this.closableViaBackdrop      = false;
        this.overlayClass             = "";
        this.containerClasses         = ["dialog-xl"];

        return this;
    }

    importExportDefaults(): OverlayConfig
    {
        this.showBackdrop             = true;
        this.showCloseButton          = true;
        this.isAnimationsEnabled      = true;
        ...
        this.closableViaBackdrop      = false;
        this.overlayClass             = "";
        this.containerClasses         = ["dialog-sm"];
