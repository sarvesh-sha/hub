...
import {DatatableColumnDirective} from "framework/ui/datatables/datatable.component";
...
export class ColumnConfiguration
{
    ...
    public template: DatatableColumnDirective;
    ...
    get activeMinWidth(): number
    {
        return this.minWidth || this.template?.minWidth;
    }
    ...
    private originalGripperOffset: number;
    private originalLeftOffset: number;

    private get gripperOffset(): number
    {
        const offset = ColumnConfiguration.resizeGripperPaddingPx + ColumnConfiguration.resizeGripperWidthPx;
        if (this.headerElement && this.template?.sortable)
        {
            if (this.originalGripperOffset) return this.originalGripperOffset - (this.leftOffset - this.originalLeftOffset);

            // sortable header cells have a different structure and an internal element with position relative for the sort arrow
            const headerPadding = UtilsService.parseComputedStyle(getComputedStyle(this.headerElement).paddingLeft, "px");
            return this.headerElement.clientWidth - headerPadding - (offset + 1);
        }

        if (this.originalGripperOffset) return this.originalGripperOffset;
        return this.leftOffset + this.flexBasis - offset;
    }
    ...
    resizePrep()
    {
        this.originalGripperOffset = this.gripperOffset;
        this.originalLeftOffset    = this.leftOffset;
    }

    completeResize()
    {
        this.originalLeftOffset    = undefined;
        this.originalGripperOffset = undefined;
    }
    ...
    updateColumnStyles()
    {
        ...
        {
            const flexBasisStyle = UtilsService.formatCssValue(this.flexBasis);
            const minWidthStyle  = UtilsService.formatCssValue(this.activeMinWidth);

            this.headerElement.style.flexBasis = flexBasisStyle;
            this.headerElement.style.minWidth  = minWidthStyle;

            for (let cellElem of this.cellElements)
            {
                cellElem.style.flexBasis = flexBasisStyle;
                cellElem.style.minWidth  = minWidthStyle;
            }
    ...
    updateGripperPosition()
    {
        ...
        {
            this.gripperElement.style.left = UtilsService.formatCssValue(this.gripperOffset);
        }
    }
