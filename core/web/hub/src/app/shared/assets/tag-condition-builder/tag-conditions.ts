...
export class ConditionNode
{
    ...
    updateById(optionId: string)
    {
        let option = ConditionNodeOptions.find((opt) => opt.optionId === optionId);
        if (option)
        {
            this.update(option);
        }
    ...
    {
        if (this.m_type !== option.type) this.value = null;
...
class ConditionNodeOption implements ITreeNode<string>
{
    public readonly optionId: string;

    public readonly children: ConditionNodeOption[] = [];
