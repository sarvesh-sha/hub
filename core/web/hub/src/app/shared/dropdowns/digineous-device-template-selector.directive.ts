...
@Directive({
               selector: "[o3DigineousDeviceTemplateSelector]"
           })
export class DigineousDeviceTemplateSelectorDirective implements ILazyLoader<string>
{
    @Input("o3DigineousDeviceTemplateSelector")
    public sink: boolean;

    private m_templates: Models.DigineousDeviceLibrary[];
    ...
    private async ensureLoaded()
    {
        if (!this.m_templates)
        {
            this.m_templates = await this.app.domain.digineous.getDeviceTemplates();
