...
import {LAYOUT_WIDTH_MD, LAYOUT_WIDTH_SM} from "framework/ui/layout";
...
export class StandardLayoutComponent extends SharedSvc.BaseApplicationComponent
{
    ...
    @ViewChild("test_toasts") test_toasts: ErrorListPaneComponent;
    @ViewChild("test_sidenavToggle", {read: ElementRef}) test_sidenavToggle: ElementRef;
    @ViewChildren("test_sidenavItem", {read: ElementRef}) test_sidenavItems: QueryList<ElementRef>;
    ...
    messageCenter()
    {
        this.app.routing.navigate(["./notification-center"]);
    }
    ...
    onResize()
    {
        ...
        else if (bodyWidth <= LAYOUT_WIDTH_MD && bodyWidth > LAYOUT_WIDTH_SM)
        {
            this.appService.setState("sidenavMode", "push");
            this.sidenavOpen = false;
        }
        else if (bodyWidth <= LAYOUT_WIDTH_SM)
        {
