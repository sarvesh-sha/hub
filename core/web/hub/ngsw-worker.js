(() =>
{
    ...
    var __defNormalProp     = (obj,
                               key,
                               value) => key in obj ? __defProp(obj,
                                                                key,
                                                                {
    ...
    var __spreadProps       = (a,
                               b) => __defProps(a, __getOwnPropDescs(b));
...
${error.stack}`;
    ...
    function arrayBufferToWords32(buffer,
                                  endian)
    {
    ...
    {
        return words32.reduce((str,
                               word) => str + word32ToByteString(word), "");
    }
    ...
    var AssetGroup         = class
    {
        ...
        {
            const cache = await this.cache;
            return (await cache.keys()).map((request) => this.adapter.normalizeUrl(request.url))
                                       .filter((url) => !this.hashes.has(url));
        }

        async fetchAndCacheOnce(req,
                                used = true)
        {
            ...
            try
            {
                ...
                {
                    throw new SwCriticalError(`Failed to update the caches for request to '${req.url}' (fetchAndCacheOnce): ${errorToString(
                        err)}`);
                }
            }
            finally
    ...
    var PrefetchAssetGroup = class extends AssetGroup
    {
        ...
        {
            const cache = await this.cache;
            await this.urls.reduce(async (previous,
                                          url) =>
                                   {
                                       await previous;
                                       const req           = this.adapter.newRequest(url);
                                       const alreadyCached = await cache.match(req,
                                                                               this.config.cacheQueryOptions) !== void 0;
                                       if (alreadyCached)
                                       {
                                           return;
                                       }
            ...
            {
                const metaTable = await this.metadata;
                await (await updateFrom.previouslyCachedResources()).filter((url) => this.urls.indexOf(url) !== -1 || this.patterns.some(
                                                                    ...
                                                                    .reduce(async (previous,
                                                                                   url) =>
                                                                            {
                                                                                ...
                                                                                const req           = this.adapter.newRequest(
                                                                                    url);
                                                                                const alreadyCached = await cache.match(
                                                                                    req,
                                                                                    this.config.cacheQueryOptions) !== void 0;
                                                                                if (alreadyCached)
                                                                                {
                                                                                ...
                                                                                const res = await updateFrom.lookupResourceWithoutHash(
                                                                                    url);
    ...
    var DataGroup = class
    {
        ...
        async handleFetchWithPerformance(req,
                                         event,
                                         lru)
        {
            let res         = null;
        ...
        async handleFetchWithFreshness(req,
                                       event,
                                       lru)
        {
            const [timeoutFetch, networkFetch] = this.networkFetchWithTimeout(req);
    ...
    var AppVersion                              = class
    {
                    ...
                    manifestHash)
        {
            ...
            this._okay            = true;
            // OPTIO3 Add new field to track initialization
            this.fullyInitialized = false;
            Object.keys(manifest.hashTable).forEach((url) =>
                                                    {
                                                        this.hashTable.set(adapter2.normalizeUrl(url),
                                                                           manifest.hashTable[url]);
                                                    });
            ...
            this.assetGroups           = (manifest.assetGroups || []).map((config) =>
                                                                          {
                                                                              switch (config.installMode)
                                                                              {
                                                                                  ...
                                                                                  case "lazy":
                                                                                      return new LazyAssetGroup(scope2,
                                                                                                                adapter2,
                                                                                                                idle,
                                                                                                                config,
                                                                                                                this.hashTable,
            ...
            this.dataGroups            = (manifest.dataGroups || []).map((config) => new DataGroup(scope2,
                                                                                                   adapter2,
                                                                                                   config,
                                                                                                   database,
                                                                                                   debugHandler,
        ...
        async initializeFully(updateFrom)
        {
            try
            {
                await this.assetGroups.reduce(async (previous,
                                                     group) =>
                                              {
                                                  await previous;
        ...
        async handleFetch(req,
                          event)
        {
            const asset = await this.assetGroups.reduce(async (potentialResponse,
                                                               group) =>
                                                        {
            ...
            const data = await this.dataGroups.reduce(async (potentialResponse,
                                                             group) =>
                                                      {
                                                          const resp = await potentialResponse;
                                                          if (resp !== null)
        ...
        isNavigationRequest(req)
        {
            ...
            return this.navigationUrls.include.some((regex) => regex.test(urlWithoutQueryOrHash)) && !this.navigationUrls.exclude.some(
                (regex) => regex.test(urlWithoutQueryOrHash));
        }

        async lookupResourceWithHash(url,
                                     hash)
        {
        ...
        {
            return this.assetGroups.reduce(async (potentialResponse,
                                                  group) =>
                                           {
                                               const resp = await potentialResponse;
                                               if (resp !== null)
        ...
        {
            return this.assetGroups.reduce(async (resources,
                                                  group) => (await resources).concat(await group.unhashedResources()),
                                           Promise.resolve([]));
        }

        async recentCacheStatus(url)
        {
            return this.assetGroups.reduce(async (current,
                                                  group) =>
                                           {
...
${msgIdle}`, {headers: this.adapter.newHeaders({"Content-Type": "text/plain"})});
    ...
    var IdleScheduler = class
    {
        ...
        async execute()
        {
            ...
            while (this.queue.length > 0)
            {
                ...
                this.queue  = [];
                await queue.reduce(async (previous,
                                          task) =>
                                   {
                                       await previous;
                                       try
    ...
    var Driver = class
    {
                    ...
                    db)
        {
            ...
            this.ngswStatePath                    = this.adapter.parseUrl("ngsw/state",
                                                                          this.scope.registration.scope).path;
        ...
        onFetch(event)
        {
            ...
            if (req.cache === "only-if-cached" && req.mode !== "same-origin")
            {
                if (!this.loggedInvalidOnlyIfCachedRequest)
                {
                    ...
                    this.debugger.log(`Ignoring invalid request: 'only-if-cached' can be set only with 'same-origin' mode`,
                          ...
                          action)
        {
            ...
            const urlToOpen          = new URL((_c = onActionClick == null ? void 0 : onActionClick.url) != null ? _c : "",
                                               this.scope.registration.scope).href;
        ...
        async completeOperation(client,
                                promise,
                                nonce)
        {
        ...
        async handleFetch(event)
        {
            ...
            try
            {
                // OPTIO3: Only allow initialized versions to handle requests
                if (appVersion !== null && appVersion.fullyInitialized)
                {
                    try
                    {
        ...
        async initialize()
        {
            ...
            try
            {
                ...
                {
                    this.debugger.log(`Missing manifest for latest version hash ${latest.latest}`,
                ...
                {
                    // OPTIO3 - Disable unnecessary update checks
                    // await this.checkForUpdate();
                });
            }
            catch (_)
            ...
            Object.keys(manifests).forEach((hash) =>
                                           {
                                               ...
                                               {
                                                   // OPTIO3: Creating a version for something that previously
                                                   // existed. Mark as initialized so it will try to handle requests.
                                                   const version            = new AppVersion(this.scope,
                                                                                             this.adapter,
                                                                                             this.db,
                                                                                             this.idle,
                                                                                             this.debugger,
            ...
            Object.keys(assignments).forEach((clientId) =>
                                             {
                                                 ...
                                                 else
                                                 {
                                                     ...
                                                     this.debugger.log(`Unknown version ${hash} mapped for client ${clientId}, using latest instead`,
            ...
            await Promise.all(Object.keys(manifests).map(async (hash) =>
                                                         {
                                                             try
                                                             {
                                                                 await this.scheduleInitialization(this.versions.get(
                                                                     hash));
                                                             }
                                                             catch (err)
                                                             {
                                                                 this.debugger.log(err,
                                                                                   `initialize: schedule init of ${hash}`);
                                                                 return false;
                                                             }
                                                         }));
        }

        lookupVersionByHash(hash,
                            debugName = "lookupVersionByHash")
        {
            if (!this.versions.has(hash))
            {
        ...
        async assignVersion(event)
        {
            ...
            if (clientId)
            {
                if (this.clientVersionMap.has(clientId))
                {
                    ...
                    if (this.state === DriverReadyState.NORMAL && hash !== this.latestHash && appVersion.isNavigationRequest(
                        event.request))
                    {
        ...
        async scheduleInitialization(appVersion)
        {
            ...
            // OPTIO3 - Keep localhost and production logic the same
        ...
        async versionFailed(appVersion,
                            err)
        {
                          ...
                          hash)
        {
            try
            {
                ...
                // OPTIO3 - Don't wait here for full initialization, just schedule the download
                await this.scheduleInitialization(this);
        ...
        async sync()
        {
            ...
            const assignments = {};
            this.clientVersionMap.forEach((hash,
                                           clientId) =>
                                          {
        ...
        async notifyClientsAboutUnrecoverableState(appVersion,
                                                   reason)
        {
            const broken = Array.from(this.versions.entries()).find(([hash, version]) => version === appVersion);
            if (broken === void 0)
        ...
        async notifyClientsAboutVersionInstallationFailed(manifest,
                                                          hash,
                                                          error)
        {
            await this.initialized;
        ...
        async notifyClientsAboutVersionDetected(manifest,
                                                hash)
        {
            await this.initialized;
            const clients = await this.scope.clients.matchAll();
        ...
        async notifyClientsAboutVersionReady(manifest,
                                             hash)
        {
            await this.initialized;
            const clients = await this.scope.clients.matchAll();
