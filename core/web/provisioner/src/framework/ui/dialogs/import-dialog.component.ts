...
export class ImportDialogComponent<T>
{
                          ...
                          readType: FileReadType = FileReadType.asText): Promise<T>
    {
        ...
        let overlayCfg                 = new OverlayConfig();
        overlayCfg.closableViaBackdrop = false;
        overlayCfg.showCloseButton     = true;
            ...
            if (files.length > 0)
            {
                ...
                else
                {
                    ...
                    reader.onload = async (e) =>
                    {
                        try
                        {
                            this.validContent = await this.data.handler.parseFile(reader.result, this.fileName);
                            this.error        = "";
                        }
                        catch (e)
                        {
