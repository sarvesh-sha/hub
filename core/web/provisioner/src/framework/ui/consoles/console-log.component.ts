...
export class ConsoleLogComponent extends BaseComponent
{
                ...
                @Optional() private expand: ExpandDirective)
    {
        ...
        this.subscribeToObservable(this.resizeSubject.pipe(debounceTime(250)), this.handleResize.bind(this));
    ...
    bind(provider: IConsoleLogProvider)
    {
        ...
        if (!this.log || this.log.provider !== provider)
        {
            if (provider)
            {
    ...
    private calculateVisibleLines(): boolean
    {
        try
        {
            ...
            if (lineHeight > 0)
            {
                ...
                linesVisible -= 1; // Account for scroll bar at the bottom.
                               ...
                               entry: IConsoleLogEntry)
    {
        ...
        if (root.subMenuItems.length > 1)
        {
            this.contextMenu.show(root.subMenuItems);
        }
    }
    ...
    private handleResize(entries: ResizeObserverEntry[])
    {
        ...
        {
            let resizer: HTMLElement = entry.target;
            ...
            {
                colConfig.width = entry.borderBoxSize.inlineSize;
            }
            this.detectChanges();
        }
    }
...
export class ConsoleLogScrollerDirective
{
    ...
    onWheelPositionChanged(event: any)
    {
    ...
    private async recomputeView()
    {
        while (true)
        {
            ...
            for (let pos = start; pos < end; pos++)
            {
                ...
                {
                    //
                    // If the section has not been fetched, load a batch of missing entries.
                    //
                    let startFetch = lineOffset;
                    let endFetch   = startFetch + 1;

                    let range = this.getFilteredRange(pos);
                    if (range)
                    {
                        endFetch  = startFetch;
                        while (!this.m_entries[endFetch + 1] && endFetch < range.endOffset) endFetch++;
                    }
                    else
                    {
                        while (endFetch < end && !this.m_entries[endFetch]) endFetch++;
                    }

                    let newEntries = await this.provider.getLogPage(startFetch, endFetch);
                    ...
                    line = this.m_entries[lineOffset];
                }
