...
export class MomentHelper
{
    ...
    public static friendlyFormatUS(value: moment.Moment | number = MomentHelper.now()): string
    {
        if (typeof value === "number") value = MomentHelper.parse(value);

        return value.format("M/D/YY [at] h:mm A");
    }
    ...
    public static toZone(value: moment.Moment,
                         zoneName: string)
    {
        return value.tz(zoneName, false);
    }
    ...
    public static formatChartTimestamp(timestamp: moment.Moment,
                                       withMs?: boolean): string
    {
        withMs = withMs ?? timestamp.valueOf() % 10 != 0;

        let formatString = "ddd MMM D ";
        if (timestamp.year() !== new Date().getFullYear()) formatString += "Y ";
