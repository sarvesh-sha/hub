...
export class BackgroundActivityDetailPageComponent extends SharedSvc.BaseComponentWithRouter
{
    ...
    async navigateToContext()
    {
        ...
        {
            case Models.DeploymentHost.RECORD_IDENTITY:
                this.app.ui.navigation.go("/deployments/item", [
                    context.sysId
                ]);
                break;

            case Models.CustomerService.RECORD_IDENTITY:
                let svc = await this.inject(CustomerServicesService)
                                    .getExtendedById(context.sysId);
