...
@Component({
               selector   : "o3-deployment-hosts-detail-page-log",
               templateUrl: "./deployment-hosts-detail-page-log.component.html"
           })
export class DeploymentHostsDetailPageLogComponent extends SharedSvc.BaseComponentWithRouter implements IConsoleLogProvider
{
    id: string;
    ...
    private logPrevious: DeploymentHostExtended;
    ...
    constructor(inj: Injector)
    {
        ...
        this.ext         = this.app.domain.deploymentHosts.wrapModel(new Models.DeploymentHost());
    }
    ...
    async loadHost()
    {
        ...
        {
            // load host info
            let host = await this.app.domain.deploymentHosts.getExtendedById(this.id);
            if (!host)
            {
                this.exit();
                return;
            ...
            this.app.ui.navigation.breadcrumbCurrentLabel = model.hostName;
    ...
    prepareDownload()
    {
        if (this.ext)
        {
            let file = this.ext.model.hostName;
...
class LogDownloader implements DownloadGenerator
{
    ...
    constructor(private extended: DeploymentHostExtended)
    {
    }
