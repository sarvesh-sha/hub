...
class LookupForTags
{
    ...
    {
        private final Map<KeyValuePair, Integer> m_fromValueToIndex = Maps.newHashMap();
    ...
    LookupForTags(MemoryMappedHeap heap)
    {
        m_heap = heap;
    }

    synchronized KeyValuePair[] accessPairs()
    {
        Data data = ensureDeserialized();

        return data.m_fromIndexToValue.toArray();
    }
    ...
    int put(KeyValuePair pair)
    {
        ...
        Integer index = data.m_fromValueToIndex.get(pair);
        if (index == null)
        {
            index = data.m_fromIndexToValue.size();
            data.m_fromIndexToValue.add(pair);
            data.m_fromValueToIndex.put(pair, index);
    ...
    Integer get(KeyValuePair pair)
    {
        Data data = ensureDeserialized();

        return data.m_fromValueToIndex.get(pair);
    }
        ...
        if (m_length != 0)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    data.m_fromIndexToValue.add(pair);
                    data.m_fromValueToIndex.put(pair, i);
                }
            }
            catch (IOException e)
            {
